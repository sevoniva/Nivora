...
| MCP response size cap | beta | JSON-RPC transport cap plus log/resource truncation | transport cap, truncation, and event/log/audit search pagination tests | MCP docs | Resource-level pagination and large-result operational guidance still need work | Add broader pagination before broad exposure | MCP |
