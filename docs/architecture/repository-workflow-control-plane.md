...
`/api/v1/workflows/run` is guarded by RBAC plus explicit `confirm=true` and `allowPipelineRun=true`. It records WorkflowRun metadata and creates a queued PipelineRun through the existing runtime. The queued PipelineRun keeps workflow/repository source IDs, including the repository snapshot ID when supplied, and generated JobRun/StepRun records keep workflow job/step IDs for audit traceability. `POST /api/v1/workflows/runs/{id}/cancel` cancels the linked PipelineRun and updates WorkflowRun metadata. `POST /api/v1/workflows/runs/{id}/retry` retries Failed, Canceled, or Timeout WorkflowRuns by converting the stored WorkflowPlan into a new queued PipelineRun, and it requires explicit confirmation. `POST /api/v1/workflows/runs/reconcile` scans non-terminal WorkflowRun records and repairs status drift from linked PipelineRun state. These routes do not execute shell steps directly and are not exposed through MCP action tools.

WorkflowPlan and WorkflowRun records are durable in configured PostgreSQL mode. The source metadata copied into PipelineRun is metadata only; raw WorkflowDefinition YAML is still not stored by this plan/run foundation because source document retention needs stronger redaction and tenant-scope review.
