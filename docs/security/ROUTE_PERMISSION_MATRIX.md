...
| GET/POST | `/api/v1/releases*` | `project.read` for reads; `release.create` for create/plan/deploy | release/project | user/service account | yes | no | yes | ReleaseExecution remains foundation-level; execution detail/timeline/target reads are guarded by stored project/environment scope. Release artifact records still need first-class project ownership metadata. |
