...
| MCP rate limit | missing | none | none | remote readiness docs | Remote abuse risk | Add before remote exposure | MCP |
| MCP response size cap | foundation | log truncation helper | truncation tests | MCP docs | General cap missing | Transport-wide cap | MCP |
