...
Artifacts bound to releases are queryable through `/api/v1/artifacts`, `/api/v1/artifacts/{id}`, `/api/v1/artifacts/{id}/releases`, and `nivora artifact list/get/releases --token-env NIVORA_AUTH_TOKEN`. This inventory is derived from ReleaseArtifact bindings that Nivora already knows about. It is not a full registry crawl.
