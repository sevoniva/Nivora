...
| MCP control plane | foundation | `cmd/nivora-mcp`, `internal/api/mcp`, `internal/app/mcp`, `make verify-mcp`, `docs/rfcs/mcp-control-plane.md` | Local stdio MCP exposes read-only resources, plan-only tools, and prompts. Action tools are blocked; runner tokens are denied; remote MCP/OAuth is future work. |
