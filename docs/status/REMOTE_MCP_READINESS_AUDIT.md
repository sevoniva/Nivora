...
Current decision: **no-go for remote MCP implementation today**. Local stdio MCP is usable for maintainer workflows. Remote read-only MCP is a future candidate after auth, tenant filtering, remote limits, and audit proof are finished.
