...
| ReleaseExecution | partial | `internal/usecase/releaseorchestration`, release API routes, `make verify-release`, `internal/adapters/repository/postgres/release_orchestration_store.go`, optional PostgreSQL recovery tests in `internal/adapters/repository/postgres/runtime_recovery_integration_test.go`, runtime recovery center aggregation | Sequential orchestration, PostgreSQL persistence, target state recovery, and stale execution queries exist; automatic ReleaseExecution mutation in recovery remains manual/report-only. |
...
| Org/project/application/environment catalog | foundation | `internal/usecase/catalog`, `/api/v1/orgs`, `/api/v1/projects`, `/api/v1/applications`, `/api/v1/environments`, `nivora org/project/application/environment` | Foundation CRUD, parent validation, enabled/disabled state, RBAC-protected API routes, CLI commands, and OpenAPI schemas exist. Current runtime wiring uses an in-memory catalog store; PostgreSQL catalog persistence and full tenant lifecycle policy remain future work. |
| SCM repository catalog | foundation | `internal/usecase/catalog`, `/api/v1/repositories`, `nivora repository`, `internal/ports/scm` | Foundation repository metadata CRUD, CredentialRef linkage, parent project validation, RBAC-protected API routes, CLI commands, and OpenAPI schemas exist. Real GitHub/GitLab/Gitea network integration remains skeleton/future work. |
...
| MCP control plane | foundation | `cmd/nivora-mcp`, `internal/api/mcp`, `internal/app/mcp`, `make verify-mcp`, `make verify-ai-control-plane`, `scripts/smoke-mcp-local.sh`, `scripts/validate-mcp-scenarios.sh`, `examples/mcp/scenarios/` (26 scenarios), `examples/mcp/golden-answers/` (26 answers), `internal/api/mcp/scenario_test.go`, `internal/api/mcp/server_test.go`, `docs/status/AI_CONTROL_PLANE_DEEP_AUDIT.md`, `docs/status/AI_OPERATOR_JOURNEYS.md`, `docs/status/MCP_ENTERPRISE_OPENING_DECISION.md`, `docs/status/MCP_ENTERPRISE_RISK_REGISTER.md`, `docs/security/MCP_PERMISSION_MATRIX.md`, `docs/security/MCP_TENANT_SCOPE_REVIEW.md`, `docs/security/mcp-threat-model.md`, `docs/rfcs/mcp-control-plane.md`, `docs/rfcs/remote-mcp-read-only.md` | Local stdio MCP exposes read-only resources, persisted evidence-bundle read access for `audit.read`, plan-only tools with `mutated=false`, prompts that treat logs/events/manifests/audit messages as untrusted evidence, denied action tools, runner-token rejection, redaction, compliance-backed audit, and validated golden operator scenarios. Remote read-only MCP is no-go until OAuth/OIDC or service-account auth, tenant filters, rate limits, response caps, pagination, and remote audit tests exist. Action MCP remains blocked. |
