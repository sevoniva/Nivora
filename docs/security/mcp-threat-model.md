...
| Replay/rate abuse | availability and audit volume | client repeats expensive reads | local stdio only; no remote endpoint | remote rate limits, request IDs, idempotency, body limits | remote DoS/rate-limit tests | block remote MCP until designed |
...
| Denial of service by large outputs | server and AI client memory | large logs or docs requested | log truncation; JSON output sanitization | global response size limits for remote | truncation and max response tests | acceptable locally; remote needs limits |
