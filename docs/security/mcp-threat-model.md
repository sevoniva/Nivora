...
| Replay/rate abuse | availability and audit volume | client repeats expensive reads | local stdio only; local request timeout, request body cap, response cap, and request rate limit exist | remote per-client rate limits, request IDs, idempotency, remote transport tests | remote DoS/rate-limit tests | block remote MCP until designed |
