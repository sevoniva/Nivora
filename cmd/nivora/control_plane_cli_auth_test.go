...
`), 0o600); err != nil {
	...
	tests := []struct {
		...
		{name: "policy results list", cmd: newPolicyResultsListCommand(), args: []string{"--policy-id", "policy-1", "--decision", "deny", "--limit", "2", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodGet, wantPath: "/api/v1/policies/results", wantQuery: "decision=deny&limit=2&policyId=policy-1", response: `{"results":[{"id":"policy-result-1"}]}`},
