...
`), 0o600); err != nil {
	...
	tests := []struct {
		...
		{name: "repository snapshot", cmd: newRepositorySnapshotCommand(), args: []string{"repo-1", "--ref", "HEAD", "--local-path", "/tmp/nivora-repo", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodPost, wantPath: "/api/v1/repositories/repo-1/snapshot", response: `{"id":"snap-1","repositoryId":"repo-1"}`},
		{name: "repository analyze", cmd: newRepositoryAnalyzeCommand(), args: []string{"repo-1", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodPost, wantPath: "/api/v1/repositories/repo-1/analyze", response: `{"repositoryId":"repo-1","snapshotId":"snap-1"}`},
		...
		{name: "repository devops plan list", cmd: newRepositoryDevOpsPlanListCommand(), args: []string{"repo-1", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodGet, wantPath: "/api/v1/repositories/repo-1/devops-plans", response: `{"plans":[]}`},
		{name: "repository devops plan get", cmd: newRepositoryDevOpsPlanGetCommand(), args: []string{"plan-1", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodGet, wantPath: "/api/v1/devops/plans/plan-1", response: `{"id":"plan-1"}`},
		{name: "repository readiness review", cmd: newRepositoryReadinessReviewCommand(), args: []string{"repo-1", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodPost, wantPath: "/api/v1/devops/readiness-review", response: `{"review":{"repositoryId":"repo-1","planOnly":true},"mutated":false}`},
		{name: "workflow list", cmd: newWorkflowListCommand(), args: []string{"--repository-id", "repo-1", "--workflow-id", "workflow-ci", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodGet, wantPath: "/api/v1/workflows", wantQuery: "repositoryId=repo-1&workflowId=workflow-ci", response: `{"workflows":[{"workflowId":"workflow-ci"}]}`},
		{name: "workflow get", cmd: newWorkflowGetCommand(), args: []string{"workflow-ci", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodGet, wantPath: "/api/v1/workflows/workflow-ci", response: `{"workflowId":"workflow-ci"}`},
		{name: "workflow draft", cmd: newWorkflowDraftCommand(), args: []string{"--repository-id", "repo-1", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodGet, wantPath: "/api/v1/repositories/repo-1/intelligence", response: `{"repositoryId":"repo-1","snapshotId":"snap-1","recommendedNivoraWorkflowDraft":"kind: Workflow"}`},
		...
		{name: "workflow reconcile", cmd: newWorkflowReconcileCommand(), args: []string{"--repository-id", "repo-1", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodPost, wantPath: "/api/v1/workflows/runs/reconcile", response: `{"scanned":1,"updated":1,"workflowRuns":[]}`},
		{name: "workflow retry", cmd: newWorkflowRetryCommand(), args: []string{"wrun-1", "--confirm", "--allow-pipeline-run", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodPost, wantPath: "/api/v1/workflows/runs/wrun-1/retry", response: `{"workflowRun":{"id":"wrun-2","status":"Queued"}}`},
		...
		{name: "pipeline dag", cmd: newPipelineInspectCommand("dag", "Get PipelineRun DAG graph metadata", "/dag"), args: []string{"prun-1", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodGet, wantPath: "/api/v1/pipeline-runs/prun-1/dag", response: `{"nodes":[],"edges":[]}`},
		{name: "pipeline jobs", cmd: newPipelineInspectCommand("jobs", "Get PipelineRun jobs", "/jobs"), args: []string{"prun-1", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodGet, wantPath: "/api/v1/pipeline-runs/prun-1/jobs", response: `[]`},
		{name: "pipeline steps", cmd: newPipelineInspectCommand("steps", "Get PipelineRun steps", "/steps"), args: []string{"prun-1", "--server", "SERVER_URL", "--token-env", "NIVORA_TEST_TOKEN"}, wantMethod: http.MethodGet, wantPath: "/api/v1/pipeline-runs/prun-1/steps", response: `[]`},
