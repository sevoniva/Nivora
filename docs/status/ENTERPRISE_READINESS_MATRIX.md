...
| Runner claim | beta | claim/lease store logic with project-scope filtering in memory and Postgres paths | claim recovery tests, project-scoped claim tests, and HTTP tenant isolation claim tests | runner protocol docs | Environment-scope and first-class runner group ownership need broader fixtures | Add first-class runner group ownership and environment-scope recovery fixtures | Runner |
