...
| Remote read-only MCP | experimental foundation | `POST /api/v1/mcp/rpc`; bearer/static-token route tests; runner-token rejection; request body cap; JSON-RPC response cap; in-process per-subject rate limit; list tool and list-like resource URI limit/offset pagination; structured unknown method/resource/tool errors; remote MCP audit attribution for allowed calls and auth-boundary denials; blocked action denial; OpenAPI contract | Broader OIDC coverage, distributed rate limits, future-resource tenant proof, and operator deployment docs remain incomplete. |
| Remote plan-only MCP | experimental foundation | plan-only local tests exist and remote JSON-RPC uses the same server dispatch | Remote plan-input ownership and abuse controls need more proof. |
...
| Response limits | body size, log truncation, capped lists | local and remote JSON-RPC response cap exists; list tools and list-like resource URIs support limit/offset pagination |
...
| pipeline/deployment/release records | yes | tenant filters for every future record family |
| logs/events/audit | limited | strict truncation, audit scope filters, distributed rate limits |
...
3. Add distributed rate-limit and operator deployment guidance for remote MCP.
4. Add remaining tenant-scope contract tests for every future remote resource/tool.
