...
| GET/POST | `/api/v1/releases*` | `project.read` for reads; `release.create` for create/plan/deploy; `deployment.cancel` for release cancel | release/project | user/service account | yes | no | yes | ReleaseExecution remains foundation-level; execution detail/timeline/target reads are guarded by stored project/environment scope. Release cancel marks the Release record and cancels non-terminal ReleaseExecutions, but does not run rollback or delete resources. New project-scoped Release records store `metadata.projectId`; older release/artifact records still need first-class project ownership metadata. |
