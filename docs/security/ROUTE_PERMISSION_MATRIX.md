...
| GET/POST | `/api/v1/approvals*` | authenticated reads; `deployment.approve` for decisions and subject resume | environment/deployment/release execution | user/service account | yes | no | yes | Decisions and `/resume-subject` are route-tested. Pending approvals cannot resume subjects; pipeline subject resume is not implemented. |
