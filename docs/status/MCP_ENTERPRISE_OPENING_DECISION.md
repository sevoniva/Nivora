...
|---|---|---|---|---|---|---|---|
| Local stdio MCP | go | local process trust, RBAC subject, redaction, audit, blocked actions, request timeout, response cap | `cmd/nivora-mcp`, `make verify-mcp`, `internal/api/mcp`, 32 scenarios, local timeout/cap tests | OS process trust remains local only | local operator asks AI for unsafe apply or secret | AI answer may be misread as execution authority | Keep enabled for maintainer workflows and demos with scripted data |
