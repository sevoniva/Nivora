...
| GET/POST | `/api/v1/pipeline-runs*` | `project.read` for list, `pipeline.run` for create/cancel | project/environment | user/service account | yes | no | yes | Shell executor is not a sandbox; run detail/log/event/timeline reads are guarded by stored project scope; optional `environmentId` records environment ownership for runner claim filtering. |
...
| POST | `/api/v1/jobs/{id}/logs`, `/api/v1/jobs/{id}/status` | `runner.manage` | runner/job | user/service account | yes | no | yes | Admin compatibility endpoints. Scoped callers may mutate only jobs whose owning PipelineRun is in their project scope. |
