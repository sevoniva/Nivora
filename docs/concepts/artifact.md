...
Known artifact references can be tracked through `POST /api/v1/artifacts` and `nivora artifact create --reference ... --token-env NIVORA_AUTH_TOKEN`. Artifacts bound to releases are also indexed and queryable through `/api/v1/artifacts`, `/api/v1/artifacts/{id}`, `/api/v1/artifacts/{id}/releases`, and `nivora artifact list/get/releases --token-env NIVORA_AUTH_TOKEN`. This catalog only contains artifacts Nivora has been told about through explicit tracking or ReleaseArtifact binding. It is not a full registry crawl.
