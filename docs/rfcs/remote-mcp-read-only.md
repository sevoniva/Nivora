...
4. Tenant-safe audit search depth across historical and unscoped records. Remote MCP audit attribution now covers client identity, request IDs, correlation IDs, transport metadata, allowed calls, auth-boundary denials, and redaction; the local/Postgres hash-chain path is already covered by `TestPostgresIntegrationMCPAuditHashChain`.
