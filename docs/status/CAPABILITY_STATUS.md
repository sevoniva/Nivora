...
| MCP control plane | foundation | `cmd/nivora-mcp`, `internal/api/mcp`, `internal/app/mcp`, `make verify-mcp`, `make verify-ai-control-plane`, `scripts/smoke-mcp-local.sh`, `scripts/validate-mcp-scenarios.sh`, `examples/mcp/scenarios/` (21 scenarios), `examples/mcp/golden-answers/` (21 answers), `internal/api/mcp/scenario_test.go`, `docs/status/AI_CONTROL_PLANE_DEEP_AUDIT.md`, `docs/status/AI_OPERATOR_JOURNEYS.md`, `docs/security/MCP_PERMISSION_MATRIX.md`, `docs/security/MCP_TENANT_SCOPE_REVIEW.md`, `docs/security/mcp-threat-model.md`, `docs/rfcs/mcp-control-plane.md`, `docs/rfcs/remote-mcp-read-only.md` | Local stdio MCP exposes read-only resources, plan-only tools with `mutated=false`, prompts that treat logs/events/manifests/audit messages as untrusted evidence, denied action tools, runner-token rejection, redaction, compliance-backed audit, and validated golden operator scenarios. Remote read-only MCP is a future candidate after OAuth/OIDC, tenant filters, limits, and remote audit tests. Action MCP remains blocked. |
