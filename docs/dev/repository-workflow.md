...
The file format uses `kind: Repository` with metadata and spec fields. Supported catalog provider values in this foundation path are `generic`, `generic_git` (normalized to `generic`), `github`, `gitlab`, `gitea`, `local`, and `archive`. `github`, `gitlab`, and `gitea` each ship a metadata-only SCMProvider skeleton under `internal/adapters/scm/{github,gitlab,gitea}` backed by the shared `internal/adapters/scm/skeleton` package. The skeletons satisfy the `scm.SCMProvider` interface without contacting any external service: credential validation is format-only, repository validation rejects inline credentials, and every network or write operation (clone, fetch, push, read file, list tree, diff refs, commit status) returns `skeleton.ErrNotImplemented`. Nivora does not call their product APIs, push commits, open pull requests, or resolve CredentialRef secret values during catalog create. Real GitLab/Gitea/GitHub Enterprise API integration is guarded future work.
