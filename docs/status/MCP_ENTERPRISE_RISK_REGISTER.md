...
| P1 | Remote rate limits are process-local and incomplete | Abuse and noisy neighbor risk | JSON-RPC has in-process per-subject request limits; distributed limits are not implemented | Add distributed rate-limit design and tests | MCP/SRE | MCP Remote Abuse Controls | medium |
| P1 | Remote audit attribution is still shallow | Weak incident reconstruction across remote clients | basic remote bearer actor/operation/decision audit test and Postgres MCP hash-chain proof exist | Add remote client identity, request/correlation id, and scope attribution tests before remote exposure | MCP/Compliance | MCP Remote Audit Contract | medium |
