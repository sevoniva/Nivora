...
| PipelineRun / Pipeline definitions | `POST /api/v1/pipeline-runs`, `GET /api/v1/pipeline-runs`, `GET /api/v1/pipeline-runs/{id}`, DAG/jobs/steps/logs/artifacts/caches/annotations/summary/events/timeline/cancel, `GET/POST/PATCH/DELETE /api/v1/pipelines`, `/api/v1/pipelines/{id}/versions`, `/api/v1/pipelines/{id}/runs` | shell runtime foundation, optional pagination on list/job/step/log/artifact/cache/annotation/event/timeline reads; PipelineRun DAG/jobs/steps are read-only views derived from stored StageRun/JobRun/StepRun records; PipelineRun metadata summary returns artifact/cache/annotation/step-summary metadata only; Pipeline definitions can be cataloged, versioned, and run from the current or a saved historical version |
| Nivora Workflow planning and guarded run | `GET /api/v1/workflows`, `GET /api/v1/workflows/{id}`, `POST /api/v1/workflows/validate`, `POST /api/v1/workflows/plan`, `GET /api/v1/workflows/{id}/plan`, `GET /api/v1/workflows/plans`, `GET /api/v1/workflows/plans/{id}`, `GET /api/v1/workflows/runs`, `GET /api/v1/workflows/runs/{id}`, `POST /api/v1/workflows/runs/{id}/cancel`, `POST /api/v1/workflows/runs/{id}/retry`, `POST /api/v1/workflows/runs/reconcile`, `POST /api/v1/workflows/run` | parser/validator/DAG planner foundation; workflow summary/list and latest-plan reads are derived from stored plan records. Plan records include content hashes, artifact/cache hints, redacted plan output, and plan-only security/release/deployment intent. Guarded run requires `confirm=true` and `allowPipelineRun=true`, records WorkflowRun metadata, queues a PipelineRun through the existing runtime, and records artifact/cache declarations as PipelineRun metadata only. WorkflowRun read APIs synchronize status from the linked PipelineRun; cancel requests cancel the linked PipelineRun and update WorkflowRun metadata without rollback/delete behavior; retry requests require confirmation and queue a replacement PipelineRun from the stored WorkflowPlan for Failed/Canceled/Timeout runs; reconcile scans non-terminal WorkflowRun records and repairs status drift from linked PipelineRun state. Raw workflow YAML is not persisted by this foundation |
...
| SCM repository catalog | `GET/POST /api/v1/repositories`, get/update/disable/validate, snapshot/intelligence, `POST /api/v1/devops/plan`, saved DevOps plan reads, `POST /api/v1/devops/readiness-review` | metadata-only repository inventory; static local/generic snapshotting, intelligence, saved DevOps plan records, and readiness review do not execute repository commands, create releases, trigger scans, deploy, contact SCM providers, or resolve CredentialRef values |
...
| Workflow execution | `POST /api/v1/workflows/run`, `POST /api/v1/workflows/runs/{id}/cancel`, `POST /api/v1/workflows/runs/{id}/retry`, `POST /api/v1/workflows/runs/reconcile` | guarded queue-only foundation; shell execution, cancellation, retry, and status reconciliation still happen through PipelineRun/Runner/Worker paths |
