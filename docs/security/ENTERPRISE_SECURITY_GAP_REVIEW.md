...
| Remote MCP controls are incomplete | Remote AI clients could enumerate or over-read state if broadly exposed | opt-in remote read-only JSON-RPC exists; local stdio cap/timeout/rate-limit tests exist; broad remote exposure remains blocked | Add tenant scope, distributed rate limit, pagination, remote timeout/cap proof, and stronger audit proof before broad remote exposure |
