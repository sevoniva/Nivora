...
The file format uses `kind: Repository` with metadata and spec fields. Supported catalog provider values in this foundation path are `generic`, `generic_git` (normalized to `generic`), `github`, `gitlab`, `gitea`, `local`, and `archive`. `gitlab` ships a real SCMProvider adapter under `internal/adapters/scm/gitlab` that calls the GitLab v4 REST API over the standard library (no third-party SDK). It authenticates with a bearer token resolved from `scm.CredentialRef` via a `TokenResolver`; without a resolver it degrades to metadata-only and returns `ErrNotConfigured` for network or write operations. Tokens are never logged or returned in errors. `gitea` and `github` each ship a metadata-only SCMProvider skeleton under `internal/adapters/scm/{gitea,github}` backed by the shared `internal/adapters/scm/skeleton` package: credential validation is format-only, repository validation rejects inline credentials, and every network or write operation returns `skeleton.ErrNotImplemented`. Nivora does not push commits, open pull requests, or resolve CredentialRef secret values during catalog create. Real Gitea/GitHub Enterprise API integration is guarded future work.
