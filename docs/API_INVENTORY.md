...
| Nivora Workflow planning and guarded run | `GET /api/v1/workflows`, `POST /api/v1/workflows/validate`, `POST /api/v1/workflows/plan`, `GET /api/v1/workflows/{id}/plan`, `GET /api/v1/workflows/plans`, `GET /api/v1/workflows/plans/{id}`, `GET /api/v1/workflows/runs`, `GET /api/v1/workflows/runs/{id}`, `POST /api/v1/workflows/runs/{id}/cancel`, `POST /api/v1/workflows/runs/reconcile`, `POST /api/v1/workflows/run` | parser/validator/DAG planner foundation; workflow summaries and latest-plan reads are derived from stored plan records. Plan records include content hashes, artifact/cache hints, redacted plan output, and plan-only security/release/deployment intent. Guarded run requires `confirm=true` and `allowPipelineRun=true`, records WorkflowRun metadata, queues a PipelineRun through the existing runtime, and records artifact/cache declarations as PipelineRun metadata only. WorkflowRun read APIs synchronize status from the linked PipelineRun; cancel requests cancel the linked PipelineRun and update WorkflowRun metadata without rollback/delete behavior; reconcile scans non-terminal WorkflowRun records and repairs status drift from linked PipelineRun state. Raw workflow YAML is not persisted by this foundation |
...
| Workflow execution | `POST /api/v1/workflows/run`, `POST /api/v1/workflows/runs/{id}/cancel`, `POST /api/v1/workflows/runs/reconcile` | guarded queue-only foundation; shell execution, cancellation, and status reconciliation still happen through PipelineRun/Runner/Worker paths |
