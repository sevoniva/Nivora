...
- Optional PostgreSQL integration tests now verify restart-style repository recovery for PipelineRun, DeploymentRun, ReleasePlan, ReleaseExecution, runner claim leases, event outbox records, RepositorySnapshot, RepositoryIntelligence, WorkflowPlan records, and guarded WorkflowRun metadata. See `docs/dev/runtime-recovery-tests.md`.
