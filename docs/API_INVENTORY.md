...
| SCM repository catalog | `GET/POST /api/v1/repositories`, get/update/disable/validate, snapshot/intelligence, `POST /api/v1/devops/plan` | metadata-only repository inventory; static local/generic snapshotting, intelligence, and DevOps planning do not execute repository commands, create releases, trigger scans, deploy, contact SCM providers, or resolve CredentialRef values |
