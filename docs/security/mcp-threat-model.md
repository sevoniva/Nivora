...
| Audit tampering | evidence chain | attacker mutates audit records or bypasses recorder | runtime MCP recorder uses compliance audit path; `TestPostgresIntegrationMCPAuditHashChain` proves Postgres hash-chain persistence for MCP audit; remote bearer calls have actor/operation/decision audit coverage | richer remote client identity, request id, and per-client audit attribution are not implemented | remote MCP client attribution and scope tests | needs hardening before broad remote use |
