...
Phase 2.3 includes a noop Argo CD provider for tests and local modeling. It does not require a local Argo CD instance.
...
The Phase 2.3 noop provider records the request but does not call a remote Argo CD API. Production sync, credentials management, RBAC, multi-cluster handling, and application history remain future work.
