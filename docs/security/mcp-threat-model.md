...
| Replay/rate abuse | availability and audit volume | client repeats expensive reads | local stdio only; local request timeout, response cap, and request rate limit exist | remote per-client rate limits, request IDs, idempotency, transport-level body limits | remote DoS/rate-limit tests | block remote MCP until designed |
...
| Denial of service by large outputs | server and AI client memory | large logs or docs requested | log truncation; JSON output sanitization; local MCP resource/tool and JSON-RPC response caps | remote pagination and request body limits | truncation and transport max-response tests | acceptable locally; remote needs limits |
