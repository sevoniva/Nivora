...
| OpenAPI / AsyncAPI / README status drift | resolved | `api/openapi/openapi.yaml` and `api/asyncapi/asyncapi.yaml` both use `0.5.0-beta-candidate` and not-production-ready descriptions. | Versioned package artifacts can still confuse readers. | Keep specs aligned during every API change. |
