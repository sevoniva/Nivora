...
| Repository catalog / intelligence | Foundation; repository metadata catalog, local/generic read-only snapshotting, static language/build/test/package detection, plan-only DevOps summaries, PostgreSQL-backed snapshot/intelligence storage in configured server/MCP mode, `nivora repository inspect`, `nivora repository devops-plan`, and repository MCP read/plan tools exist; external SCM writes remain future work |
