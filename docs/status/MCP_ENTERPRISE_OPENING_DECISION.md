...
|---|---|---|---|---|---|---|---|
| Local stdio MCP | go | local process trust, RBAC subject, redaction, audit, blocked actions, request timeout, response cap | `cmd/nivora-mcp`, `make verify-mcp`, `internal/api/mcp`, 31 scenarios, local timeout/cap tests | OS process trust remains local only | local operator asks AI for unsafe apply or secret | AI answer may be misread as execution authority | Keep enabled for maintainer workflows and demos with scripted data |
| Remote read-only MCP | experimental opt-in foundation | OAuth/OIDC or service-account bearer, tenant filters, resource ownership checks, rate limit, timeout, request/response cap, pagination, remote audit metadata | `POST /api/v1/mcp/rpc`, bearer/static-token route tests, runner-token denial, shared timeout/cap helpers, in-process per-subject rate-limit tests, list tool and list-like resource URI pagination tests, structured unknown method/resource/tool tests, remote MCP audit attribution for actor/operation/request/correlation/client/transport metadata, and Postgres MCP hash-chain proof exist | broader OIDC/service-account contract tests, tenant-filtered fixtures for every future resource, distributed rate limits, tenant-safe audit search depth | IDOR against run IDs, broad audit search, log scraping | Could expose cross-tenant delivery metadata if enabled too broadly | Keep disabled by default; use only controlled experiments until remaining tests pass |
...
| Response size cap | foundation | shared MCP response cap config and tests exist | large-result scenarios still need more depth |
| Pagination | partial | list tools and list-like resource URIs support limit/offset pagination | large-result and future-resource scenarios still need work |
| Audit chain | partial | compliance recorder, audit hash tests, remote actor/operation/request/correlation/client attribution, and `TestPostgresIntegrationMCPAuditHashChain` exist | tenant-safe audit search depth and distributed audit ingestion behavior still need more proof |
...
- logs are returned without pagination or truncation/redaction;
...
Use MCP as a **local read-only and plan-only operator assistant** and treat remote read-only MCP as an explicitly enabled experiment only. Do not broadly expose it until tenant-scoped read-only resources, distributed rate limits, large-result scenarios, and tenant-safe audit search depth are implemented and tested. Action MCP remains blocked.
