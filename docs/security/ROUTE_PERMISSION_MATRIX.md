...
| POST | `/api/v1/repositories/{id}/validate` | `project.read` | project/repository | user/service account | yes | no | yes | Metadata-only validation; does not contact SCM providers or resolve CredentialRef values. |
...
| GET/POST | `/api/v1/deployments*` | `project.read` for reads; `deployment.create`, `deployment.cancel`, or `deployment.approve` for mutations | environment/target | user/service account | yes | no | yes | Apply/sync/rollback remain guarded and not default; run detail/log/event/timeline/resource reads are guarded by stored project/environment scope. GitOps plans may resolve `target.repositoryId` against project-scoped repository catalog metadata only; no SCM provider contact or CredentialRef value resolution occurs. Host dry-run/noop can use `POST /api/v1/deployments`; CLI server-backed host run rejects remote apply inputs. |
