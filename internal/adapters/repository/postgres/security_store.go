...
func (s *SecurityStore) Save(ctx context.Context, record securityusecase.ScanRecord) error {
	...
	_, err := s.pool.Exec(ctx, `INSERT INTO security_scans (id, subject_type, subject_id, project_id, environment_id, scanner, status, summary_total, summary_low, summary_medium, summary_high, summary_critical, findings, policy_decision, policy_reason, policy_findings, signature_subject, signature_status, signature_result, signature_key_ref, signature_identity, signature_issuer, sbom_format, sbom_storage_ref, sbom_digest, warnings, started_at, finished_at, created_at)
...
func (s *SecurityStore) Get(ctx context.Context, id string) (securityusecase.ScanRecord, error) {
	...
	err := s.pool.QueryRow(ctx, `SELECT id, subject_type, subject_id, project_id, environment_id, scanner, status, summary_total, summary_low, summary_medium, summary_high, summary_critical, findings, policy_decision, policy_reason, policy_findings, signature_subject, signature_status, signature_result, signature_key_ref, signature_identity, signature_issuer, sbom_format, sbom_storage_ref, sbom_digest, warnings, started_at, finished_at, created_at FROM security_scans WHERE id=$1`, id).
		Scan(&rec.Scan.ID, &rec.Scan.SubjectType, &rec.Scan.SubjectID, &rec.Scan.ProjectID, &rec.Scan.EnvironmentID, &rec.Scan.Scanner, &rec.Scan.Status, &rec.Scan.Summary.Total, &rec.Scan.Summary.Low, &rec.Scan.Summary.Medium, &rec.Scan.Summary.High, &rec.Scan.Summary.Critical, &findingsJSON, &rec.Policy.Decision, &rec.Policy.Reason, &policyFindingsJSON, &rec.Signature.Subject, &rec.Signature.Status, &rec.Signature.Result, &rec.Signature.KeyRef, &rec.Signature.CertificateIdentity, &rec.Signature.Issuer, &rec.SBOM.Format, &rec.SBOM.StorageRef, &rec.SBOM.Digest, &warningsJSON, &rec.Scan.StartedAt, &rec.Scan.FinishedAt, &rec.Scan.CreatedAt)
...
func (s *SecurityStore) List(ctx context.Context) ([]securityusecase.ScanRecord, error) {
	rows, err := s.pool.Query(ctx, `SELECT id, subject_type, subject_id, project_id, environment_id, scanner, status, summary_total, summary_low, summary_medium, summary_high, summary_critical, findings, policy_decision, policy_reason, policy_findings, signature_subject, signature_status, signature_result, signature_key_ref, signature_identity, signature_issuer, sbom_format, sbom_storage_ref, sbom_digest, warnings, started_at, finished_at, created_at FROM security_scans ORDER BY created_at`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
