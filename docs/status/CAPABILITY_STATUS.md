...
| MCP control plane | foundation | `cmd/nivora-mcp`, `internal/api/mcp`, `internal/app/mcp`, `make verify-mcp`, `scripts/smoke-mcp-local.sh`, `examples/mcp/scenarios/`, `internal/api/mcp/scenario_test.go`, `docs/security/MCP_PERMISSION_MATRIX.md`, `docs/security/mcp-threat-model.md`, `docs/rfcs/mcp-control-plane.md`, `docs/rfcs/remote-mcp-read-only.md` | Local stdio MCP exposes read-only resources, plan-only tools with `mutated=false`, prompts, denied action tools, runner-token rejection, redaction, compliance-backed audit, and golden operator scenarios. Remote read-only MCP is a future candidate after OAuth/OIDC, tenant filters, limits, and remote audit tests. Action MCP remains blocked. |
