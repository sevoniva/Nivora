...
| Repository catalog / intelligence | Foundation; repository metadata catalog, file-based `nivora repository create --file`, local/generic read-only snapshotting, static language/build/test/package detection, plan-only DevOps summaries, PostgreSQL-backed snapshot/intelligence storage in configured server/MCP mode, `nivora repository inspect/snapshot/analyze/devops-plan`, and repository MCP read/plan tools exist; external SCM writes remain future work |
| Nivora Workflow | Foundation; `.nivora/workflows/*.yaml` parser, validator, DAG/matrix planner, artifact/cache hints, plan-only security/release/deployment intent, Pipeline definition conversion, stored plan records, guarded WorkflowRun metadata, `nivora workflow validate/plan/run/cancel/reconcile/retry`, and plan-only API/MCP surfaces exist; WorkflowRun can queue/cancel/retry linked PipelineRun records, reconcile status from linked PipelineRun state, and record artifact/cache metadata, but it is not a full workflow engine |
