...
| MCP control plane | foundation | `cmd/nivora-mcp`, `internal/api/mcp`, `internal/app/mcp`, `make verify-mcp`, `scripts/smoke-mcp-local.sh`, `docs/security/MCP_PERMISSION_MATRIX.md`, `docs/rfcs/mcp-control-plane.md` | Local stdio MCP exposes read-only resources, plan-only tools, prompts, denied action tools, runner-token rejection, redaction, and compliance-backed audit. Remote MCP/OAuth and action tiers are future work. |
