...
Current maturity: **near-production-candidate (0.9.0-rc.1), not production-ready**. All 11 runtime stores have PostgreSQL implementations. Audit hash chaining is wired into all governance stores. Runtime store audit paths (pipeline/deployment/release) use per-store audit tables.
