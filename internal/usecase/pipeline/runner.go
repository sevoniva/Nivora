...
type Runner interface {
	ID() string
	RunShellStep(ctx context.Context, jobRunID string, command string, env map[string]string, timeout time.Duration) (executor.Result, error)
}
...
func (r *LocalRunner) RunShellStep(ctx context.Context, jobRunID string, command string, env map[string]string, timeout time.Duration) (executor.Result, error) {
