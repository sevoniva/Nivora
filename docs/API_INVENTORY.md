...
| Nivora Workflow planning and guarded run | `GET /api/v1/workflows`, `POST /api/v1/workflows/validate`, `POST /api/v1/workflows/plan`, `GET /api/v1/workflows/{id}/plan`, `GET /api/v1/workflows/plans`, `GET /api/v1/workflows/plans/{id}`, `GET /api/v1/workflows/runs`, `GET /api/v1/workflows/runs/{id}`, `POST /api/v1/workflows/run` | parser/validator/DAG planner foundation; workflow summaries and latest-plan reads are derived from stored plan records. Plan records include content hashes, artifact/cache hints, redacted plan output, and plan-only security/release/deployment intent. Guarded run requires `confirm=true` and `allowPipelineRun=true`, records WorkflowRun metadata, queues a PipelineRun through the existing runtime, and records artifact/cache declarations as PipelineRun metadata only. WorkflowRun read APIs synchronize status from the linked PipelineRun. Raw workflow YAML is not persisted by this foundation |
