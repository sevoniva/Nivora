...
| Runner claim | beta | claim/lease store logic with project/environment-scope filtering and RunnerGroup constraints in memory and Postgres paths | claim recovery tests, project/environment-scoped claim tests, RunnerGroup claim tests, and HTTP tenant isolation claim tests | runner protocol docs | Fleet-scale scheduling and OS-level sandboxing remain out of scope | Add runner fleet load tests and isolated executor profiles | Runner |
...
| Service account | foundation | auth model/store | focused tests, HTTP lifecycle route tests, and Postgres token lifecycle test | auth docs | Token lifecycle analytics limited | Operational distribution and audit reporting drill | Security |
| API token | foundation | token metadata/hash paths and CLI expiry flag | token tests, HTTP create/rotate/revoke/expire route tests, CLI expiry request tests, and Postgres rotation/revoke persistence test | auth docs | Use analytics limited | Token use analytics and audit evidence expansion | Security |
...
| Tenant isolation | foundation | tenant tests, scoped MCP artifact/security/runner filters, project/environment-scoped runner claim filtering, and RunnerGroup scope constraints | route tenant tests, runner claim scope tests, runner group scope tests, and MCP scope tests | tenancy docs | Broad metadata surfaces still need remote-safe profiles | Remote-safe metadata profile and negative tests for future remote APIs | Security |
...
| MCP response size cap | beta | JSON-RPC transport cap plus log/resource truncation | transport cap, truncation, and event/log/audit/artifact/security/evidence pagination tests | MCP docs | Remaining list-like resources and large-result operational guidance still need work | Add broader pagination before broad exposure | MCP |
