...
| GET/POST | `/api/v1/pipeline-runs*` | `project.read` for list, `pipeline.run` for create/cancel | project/environment | user/service account | yes | no | yes | Shell executor is not a sandbox; run detail/log/event/timeline reads are guarded by stored project/environment scope; optional `environmentId` records environment ownership for runner claim filtering. |
