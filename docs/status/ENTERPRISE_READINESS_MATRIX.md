...
| Runner claim | beta | claim/lease store logic with project/environment-scope filtering in memory and Postgres paths | claim recovery tests, project/environment-scoped claim tests, and HTTP tenant isolation claim tests | runner protocol docs | First-class runner group ownership needs broader fixtures | Add first-class runner group ownership and recovery fixtures | Runner |
