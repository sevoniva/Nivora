...
| MCP repository/workflow tools | foundation | `nivora_repository_inspect`, `nivora_repository_devops_plan`, `nivora_workflow_validate`, `nivora_workflow_plan`, `nivora://repositories/{id}/devops-plan`, `nivora://workflows/runs`, `nivora://workflows/runs/{id}` | Plan-only tools and read-only resources return `mutated=false`; WorkflowRun MCP resources expose guarded metadata only. Destructive MCP actions remain blocked. |
...
- MCP repository/workflow tools and guarded WorkflowRun metadata resources
