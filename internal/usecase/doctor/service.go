...
func CheckConfig(cfg config.Config) Report {
	...
	report.add(checkBool("audit.persistence", "audit", statusFor(!production || cfg.Database.RuntimeStore == "postgres", StatusPass, StatusFail), "audit and evidence persistence require PostgreSQL for production-like installs", "set database.runtime_store: postgres and run migration validation", cfg.Database.RuntimeStore))
	report.add(checkBool("event_outbox.persistence", "runtime", statusFor(!production || cfg.Database.RuntimeStore == "postgres", StatusPass, StatusFail), "event outbox recovery requires PostgreSQL for production-like installs", "set database.runtime_store: postgres and verify runtime recovery tests", cfg.Database.RuntimeStore))
	report.add(checkBool("runner.identity_config", "runners", statusFor(cfg.Runner.Name != "" && cfg.Runner.Group != "" && cfg.Runner.HeartbeatInterval != "", StatusPass, StatusFail), "runner identity and heartbeat config must be explicit", "set runner.name, runner.group, and runner.heartbeat_interval", cfg.Runner.Name+"/"+cfg.Runner.Group))
	report.add(Check{ID: "secret_provider.posture", Area: "security", Status: StatusNotChecked, Reason: "secret provider safety depends on runtime secret storage and deployment environment", Remediation: "verify secret provider configuration and keep secret values out of config, logs, audit, events, and responses"})
	report.add(Check{ID: "runner.token_storage", Area: "runners", Status: StatusNotChecked, Reason: "runner token hash storage requires live runtime state", Remediation: "verify runner registration returns the raw token once and later APIs never return token hashes"})
	report.add(Check{ID: "api.openapi_route_contract", Area: "api", Status: StatusNotChecked, Reason: "OpenAPI route coverage is verified by route contract tests rather than the local config doctor", Remediation: "run the route contract tests or make verify"})
	report.add(Check{ID: "security.secret_scan", Area: "security", Status: StatusNotChecked, Reason: "repository secret scanning is an external verification step", Remediation: "run ./scripts/verify-no-secrets.sh or make verify"})
